...
## D-039 · 2026-07-13 · DD-037 Phase 4 play space — drawer aggregation, gallery source-data pattern, Horizon tap-to-toggle, tile flip

**Decision:** the browse layer (shelf + war-chest drawer + gallery + Horizon panels) lands as the default `#/` route, replacing the plain Cases/Field-Clerks list Phase 2/3 shipped there. The reading-room document routes (`#/fc/<id>`, `#/case/<id>`) are untouched code paths — `renderDocument`, `renderSections`, `parseBlocks`, `renderInline` were not edited. Landing gets tile 06, live, `accent: "ink"` (reused, no new palette).

**Exporter growth (the one seam-touching change set, both parts pre-approved):**
- `source_slug` (the source file's stem) added to every case/field-clerk manifest entry. Most corpus files are docket-named so this is a harmless alias of `id`; `trump_v_cook.md` and `trump_v_slaughter.md` are named descriptively and their docket ids collide with their paired Field Clerks (both share docket `25A312` / `25-332`). The corpus's own cross-refs use the descriptive slug (`[[trump_v_cook|...]]`), which Phase 2's id-only lookup could never resolve — it silently degraded to plain text. `source_slug` gives the renderer a second lookup key that disambiguates cleanly (verified live: "Trump v. Slaughter" inside the Cook case now resolves to `#/case/25-332` instead of degrading).
- `warchest.json`: aggregates every Public field-clerk's `war_chest_candidates` array into named specimens. Name = the raw entry, lowercased, with a trailing `-CATEGORY` marker stripped (`backdrop-rebuttable-presumption-CATEGORY` -> `backdrop-rebuttable-presumption`, matching the kickoff's own naming of it as the inaugural specimen — it also sorts first alphabetically). `from_fcs` = the FC docket ids that named it; `count` = its length. Current corpus (5 Public FCs) produces 20 specimens, all singleton (no two FCs happen to phrase the same doctrine identically yet) — the aggregation is real and will merge naturally once a future FC repeats a name.

**Judgment call, flagged rather than guessed past:** the locked pre-decisions named `source_slug` and `warchest.json` as the two approved exporter changes, but the shelf's "3 most recent Public docs" (pre-decision 3) has no field to sort by — field-clerk frontmatter carries `sr_review_date`, case frontmatter carries `decision_date`, and neither was in the Phase 1/2 manifest schema. Added a third small derived field, `updated_at` (`sr_review_date` if present, else `decision_date`, `str()`-normalized since YAML parses unquoted dates as `date` objects), following the same Standing-Rule-#2 derivation logic already ratified for `id`/`title` (D-036) and `source_slug` above. Flagging this because it's a schema addition beyond the two the kickoff named explicitly, not because it's blocked — it's the same shape of move, done for the same reason.

**The drawer:** war-chest specimens render as index-card objects in a CSS grid — mono small-caps name (`font-variant: small-caps` on JetBrains Mono), a gold-text count (light role, never a fill), and a native `<button aria-expanded>` that reveals the card's `from_fcs` as in-page links (`#/fc/<id>`) on click/tap, collapsed by default. No hover dependence.

**The shelf:** cases and field-clerks are merged, sorted descending by `updated_at` (string comparison after normalizing `/` to `-`, so `sr_review_date`'s `YYYY-MM-DD` and `decision_date`'s `YYYY/MM/DD` compare correctly against each other), and the top 3 render as horizontal hero bands (kind-label + docket, title, one-line excerpt of the first section's first sentence, markdown-stripped by a small dedicated `stripMarkdownForExcerpt`/`excerptFor` pair — independent of the reading room's own `renderMarkdown` pipeline, so the document templates stay untouched). The remaining docs render in the existing grouped list below, with the 3 shelf entries excluded (`buildIndexGroup` gained an `excludeIds` parameter) rather than duplicated.

**The gallery:** JR approved 1 of 3 proposed LRD-004 concept images (Concept 1, "The Demise of the Counterpane"; Concepts 2 and 3 held back — Concept 3 specifically flagged for garbled AI-generated background text and a caption that doesn't parse). `sr-playspace/data/gallery.json` is hand-authored source data (not exporter-compiled, per pre-decision 4), with a hand-maintained `gallery.js` global sibling following the existing file:// fallback convention. Section ships `hidden` automatically whenever `state.gallery` is empty — the fallback path the pre-decision specifies, exercised structurally (not just claimed) since it's a real `if (!state.gallery.length) { section.hidden = true; }` branch.

**Horizon Axis (Ms.G's parked research, re-entering per the Phase-3 packet's ruling):** two `<button aria-expanded>` panels ("Research Repositories", "Radar Intel"), tap/click-driven, collapsed by default, `hidden` toggled on the linked `<ul>` — no hover-dependent reveal (Ms.G's original hover-dropdown CSS was not used; this is a fresh implementation against the redesign ruling). All 9 destination links (CourtListener, Justia, Supreme Court, Google Scholar, USAspending, PACER/PCL, and the three podcasts) point at URLs verified live via web search this session, not guessed — external links, `target="_blank"` + `rel="noopener"`, no API calls, no tracking.

**Hotfix (pre-existing, discovered by this phase's own mobile real-content check):** `.pw-inline-code` had no wrap rule; the Cook FC's Cross-references section contains long unbroken vault paths in backtick spans that overflowed 375px (scrollWidth 482 vs. clientWidth 375), pre-dating this phase (Phase 2/3 CSS, untouched otherwise). Added one property, `overflow-wrap: anywhere`, to `.pw-inline-code` — the narrowest fix, affects only strings too long to fit their line, byte-identical for every span that already fit. Re-verified all 9 routes (browse + 8 docs) at both 375px and 1280px after the fix: zero horizontal scroll everywhere.

**The sixth tile:** `landing.jsx` PROJECTS array gains `n: "06"`, kicker "Sharpen Reason · Legal", title "SR Play-Space" (placeholder, JR may rename), blurb "A reading room, and the walk to it.", `href: "sr-playspace/index.html"`, `status: "live"`, `accent: "ink"` (reused from the museum tile — no new `TileArt` case, no new landing palette, per guardrail). Landing stays SoD dark; regal begins on entry to the play-space, unchanged from Phase 3.

**Verified this session:**
- Live-rendered at `localhost:8177` (Python `http.server` against `quick-front-end/shade-of-design-landing/`): landing tile 06 renders `<a>`, live, `target="_blank"`, `rel="noopener noreferrer"`; browse route shows 3 shelf bands (25-332, 25A312, 21-1729 — correct by `updated_at` descending), 20 drawer specimens, 1 gallery image, 2 Horizon panels; grouped lists below correctly exclude the 3 shelf ids.
- Drawer card click: `aria-expanded` flips `false`->`true`, panel `hidden` flips, correct `from_fcs` link (`backdrop-rebuttable-presumption` -> `#/fc/25A312`).
- Horizon panel click: `aria-expanded` flips, all 4 "Research Repositories" links present with correct `href`/`target`/`rel`.
- Cross-ref fix, live: Cook case (`#/case/25A312`) renders zero unresolved `.pw-xref` spans; "Trump v. Slaughter" resolves to `#/case/25-332` (previously degraded to plain text).
- All 9 routes (browse + 8 docs), 375px and 1280px: `scrollWidth === clientWidth` in every case, post-hotfix.
- Token lint (same grep-based check as D-038) re-run against the grown `playspace.css`: **PASS**, zero raw hex/rgb/rgba outside `:root`.

**Files touched:**
- `pipeline/legal/export_manifests.py` — `source_slug`, `updated_at`, `_kebab_name`, `_iter_public` refactor, `_export_warchest`, warchest.json/.js output
- `quick-front-end/shade-of-design-landing/sr-playspace/data/{cases,field-clerks}.json` + `.js` — regenerated, not hand-edited
- `quick-front-end/shade-of-design-landing/sr-playspace/data/warchest.json` + `.js` — new, exporter-generated
- `quick-front-end/shade-of-design-landing/sr-playspace/data/gallery.json` + `.js` — new, hand-authored source data
- `quick-front-end/shade-of-design-landing/sr-playspace/assets/lrd-004-concept-1.png` — new, JR-approved
- `quick-front-end/shade-of-design-landing/sr-playspace/index.html` — 2 new `<script>` tags (warchest.js, gallery.js fallback siblings)
- `quick-front-end/shade-of-design-landing/sr-playspace/playspace.js` — `buildLookup`/`renderCrossRef` slug-aliasing fix, browse-layer render functions (shelf/drawer/gallery/Horizon), manifest loading grown to 4 sources
- `quick-front-end/shade-of-design-landing/sr-playspace/playspace.css` — browse-layer styles (token-only, lint-passing) + the `.pw-inline-code` overflow-wrap hotfix
- `quick-front-end/shade-of-design-landing/landing.jsx` — tile 06
- No corpus files touched; no visibility fields touched

**Review fix (Mr.C desk, 2026-07-13, during closeout):** the shelf exclusion was keyed by bare id (`shelf.recentIds` passed to both index groups), but a paired case and Field Clerk share a docket id (25A312, 25-332) — shelving the two FCs silently removed the Trump v. Cook and Trump v. Slaughter *case syntheses* from the browse route entirely (6 of 8 Public docs reachable, not 8). Fix: exclusion keys are now `kind + ":" + id` (`recentKeys`), so a shelved FC no longer hides its docket-sharing case. Verified live post-fix: all 8 Public docs reachable from browse (3 shelf bands + 3 cases + 2 FCs), no duplicates, no horizontal scroll at 375px. Same id-collision family that motivated `source_slug` — worth remembering: any browse-layer keying on document identity must carry kind, the docket id alone is not unique across kinds.

---

## D-040 · 2026-07-14 · DD-037 Phase 5 cadence + vault-native variant — one-code-path sync wrapper, no-auto-git boundary, vault-index-by-generation, chain convention

**Decision:** the map's closing phase ships as plumbing, not a new surface. One wrapper script (`pipeline/legal/sync_sr.py`) is the ONLY entry point either invocation path uses — the on-demand "SYNC SR" trigger and the daily scheduled task both execute the literal string `python pipeline/legal/sync_sr.py`. No flag forks behavior; no second export function exists. The wrapper calls `export_manifests.run_export()`, snapshots every byte-emitting artifact before and after the call to report `changed=yes|no` (the idempotency signal a no-op day should show), and appends one line to `pipeline/legal/sync_sr.log`. It performs zero git operations — no add, no commit, no push, not even a `git status` shell-out. The sync run regenerates the working tree and stops; JR's commit + merge to main remains the sole publish act, per the DD-037 standing human-review gate.

**Scheduling artifact, not scheduling action:** `pipeline/legal/sync_sr_task.md` carries the exact `schtasks /create` one-liner (absolute paths — resolved via `sys.executable` rather than the `WindowsApps\python.exe` PATH-alias stub, since Task Scheduler does not resolve PATH the way an interactive shell does) plus the matching `/delete` line. C-Build authored it; C-Build did not run `/create` — JR installs the task by his own hand, same reasoning as the Phase-4 gallery-approval gate (system settings are his). Cadence is `/sc daily`, a **ceiling** per Fable's wall: the corpus moves at JR's curation speed, not faster. Verified without installing the task: the exact inner command line (`python.exe` absolute path + `sync_sr.py` absolute path) was run from a fresh PowerShell process — genuinely a different process from the desk's working shell, the closest available proxy for "a cold shell, not your dev shell" — and produced the same log line the trigger path does.

**Vault-native index is generated, not authored:** `_SR-INDEX.md` lands at the corpus root (`research/data/legal/_SR-INDEX.md`), written by a new `run_export()` step, never under `sr-playspace/data/` and never referenced by any site manifest. Unlike the site exporters (`_iter_public`, `_build_entry`), the index scan does **not** gate on visibility or apply the Public-only schema-validation block — it lists every corpus doc, Public and Private, because the boundary governs the *site* export, not JR's own vault (DD-037 Standing Rule #6, read literally: "the exporter emits to the public manifest only files explicitly marked Public" — `_SR-INDEX.md` is not that manifest). A file with malformed frontmatter or no H1 still gets a best-effort line (title/docket fall back to the filename stem) rather than aborting the whole sync over a Private note nobody is publishing; a Public file failing that same parse still aborts the site-manifest half of the run exactly as before, since `_iter_public`/`_build_entry` are untouched. Frontmatter carries `cssclasses: [sod-room, sod-records]` and the body opens with an explicit auto-generated notice; plain `[[wikilink]]` list items only, no dataview/dataviewjs, per standing law.

**Judgment call, flagged rather than guessed past — wikilink target had to be folder-qualified.** Three corpus pairs share an exact filename across `cases/` and `field-clerks/` (`21-1729.md`, `24-1990.md`, `24-2242.md` each exist in both directories, same docket, same basename — the same id-collision family D-039 already named for `source_slug`). A bare `[[21-1729]]` wikilink is ambiguous in Obsidian across two files with an identical basename. Fix: every index line links `[[<folder>/<stem>|<title>]]` (e.g. `[[cases/21-1729|...]]` vs. `[[field-clerks/21-1729|...]]`) instead of a bare stem — still a plain markdown wikilink, no dataview, fully disambiguated. Not one of the two pre-decisions named explicitly, but the same shape of move as `source_slug`: the seam meeting real corpus collision, handled at the seam.

**`source_url` passthrough (pre-decision 4, JR's yes on record 2026-07-14):** added to `_build_entry` unconditionally rather than gated by directory — case frontmatter carries `source_url` today, field-clerk frontmatter never has the key, so FC entries simply serialize `source_url: null` with zero extra branching. One shared function, one schema, the same Standing-Rule-#2 shape as `id`/`title`/`source_slug`/`updated_at`. The renderer, not the exporter, enforces "cases only, no FC source line": `playspace.js`'s `renderDocument` only appends `.pw-source-line` when `kind === "case" && entry.source_url`, so a case missing the field renders no line — graceful absence, not a validation error, exactly as specified. Styled muted register only (`--sr-muted-text`, `--sr-hairline` underline, `--sr-regal-soft` on hover/focus) — no gold, since gold stays a light/structural role per the Phase-3 law, never a plain-text link color.

**Chain documentation (pre-decision 6, DD-026 unlocked by its 2026-07-11 live smoke):** `pipeline/PIPELINE.md` gained a "The standing chain" section — DD-026 fetch → DD-029 synthesis → corpus (JR visibility) → DD-037 export → JR merge → deployed. Documentation only; zero lines of `export_manifests.py` or `sync_sr.py` reference DD-026 or its outputs. The exporter reads `research/data/legal/{cases,field-clerks}/` regardless of how a file arrived — the auto-fetch chain, a manual `LR Chain` invocation, or JR hand-authoring a case.md all look identical to the exporter, which is the whole point of the guardrail.

**Verified this session:**
- `python pipeline/legal/export_manifests.py` re-run against the real corpus: 3 cases, 5 field-clerks, 20 warchest specimens, 17 `_SR-INDEX.md` entries (12 cases + 5 FCs). Re-run a second time immediately after: `_SR-INDEX.md` byte-identical (`diff` clean) — the index generation is as idempotent as the JSON manifests.
- `python pipeline/legal/sync_sr.py` run twice in sequence: both report `changed=no` (correct — the corpus hadn't moved between runs), both append a line to `sync_sr.log`, `git status` in `Terminal/Central Hub/` shows zero staged or committed changes after either run (only the pre-existing unstaged edits from this session's own file edits, nothing sync_sr.py itself touched via git).
- Cold-shell proof: the literal `/TR` inner command line, run via PowerShell (a separate process from the desk's working Bash shell), printed the same `[SYNC SR] ... OK ... changed=no` line and appended to the same log — the exact command the scheduled task will execute works standalone.
- Case-route source line, live at `localhost:8177`: `#/case/25A312` and `#/case/25-332` (both `supremecourt.gov` PDFs) and `#/case/26-1575` (a `courtlistener.com` docket URL, the ORDER-shape document) all render a `<p class="pw-source-line">` containing `<a href="..." target="_blank" rel="noopener">Source: CourtListener</a>` with computed color `rgb(150, 167, 185)` (`--sr-muted-text`, not gold). All 5 `#/fc/<id>` routes render no `.pw-source-line` element at all. The browse route (`#/`) also has none.
- All 9 routes (browse + 3 cases + 5 FCs) swept at 375px: `document.documentElement.scrollWidth === clientWidth` in every case — the added header line does not introduce overflow.
- `renderDocument`/`renderSections`/`parseBlocks`/`renderInline`/`escapeHtml` — zero lines touched beyond the one additive `if (kind === "case" && entry.source_url)` block in `renderDocument`; Gate D (Phase 2/3 byte-identical reading room) holds by inspection, same standard as D-039.

**Files touched:**
- `pipeline/legal/export_manifests.py` — `source_url` passthrough in `_build_entry`; new `_SR_INDEX_FILENAME` constant, `_index_entry`/`_index_group`/`_index_lines`/`_render_sr_index`/`_write_sr_index`; `run_export()` return grows an `index_entries` count
- `pipeline/legal/sync_sr.py` — new, the one-code-path wrapper
- `pipeline/legal/sync_sr_task.md` — new, schtasks create/delete lines for JR's hand
- `pipeline/legal/sync_sr.log` — new, append-only run log (created on first run)
- `pipeline/PIPELINE.md` — new "The standing chain" section
- `research/data/legal/_SR-INDEX.md` — new, exporter-generated, vault root (not a corpus edit — a new file)
- `quick-front-end/shade-of-design-landing/sr-playspace/data/{cases,field-clerks}.json` + `.js` — regenerated (grew the `source_url` field), not hand-edited
- `quick-front-end/shade-of-design-landing/sr-playspace/playspace.js` — one additive block in `renderDocument`
- `quick-front-end/shade-of-design-landing/sr-playspace/playspace.css` — `.pw-source-line` rules, token-only
- No corpus files touched; no visibility fields touched; no git operations by any script

**Review fix (Mr.C desk, 2026-07-15, during closeout):** the source-line label was hardcoded "Source: CourtListener" per the kickoff's literal spec — but the corpus sources from more than one archive (Cook and Slaughter carry `supremecourt.gov` PDF URLs), so the literal label mislabeled 2 of the Public cases. The spec error was Mr.C's, C-Build followed it and flagged it rather than re-litigating — correct lane behavior on both counts. Fix: label derives from the URL's own hostname (`Source: courtlistener.com` / `Source: supremecourt.gov`, `www.` stripped, try/catch to a bare "Source" on an unparsable URL). Verified live on all 4 Public case routes post-fix; FC routes and browse still render no source element; 10 routes clean at 375px.

**Post-ship corpus event (JR's hand, 2026-07-14 ~08:00 EDT, after C-Build's session):** `cases/21-1729.md` marked `visibility: Public` via Obsidian (frontmatter re-serialized in Obsidian's block style) and the SYNC SR trigger run against it — log shows `cases=4 changed=yes` then `changed=no`. This is the Gate-F trigger leg walking on a real corpus event, exactly as designed. **Data flag, pre-merge:** 21-1729's `source_url` is a literal placeholder (`.../opinion/<placeholder>/...`) — renders a broken source link. One-line frontmatter fix (paste the real CourtListener opinion URL, or blank the field for graceful absence) before JR merges.
